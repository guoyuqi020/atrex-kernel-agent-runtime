...
包含只读 Agent Source、当前 Kernel、公开问题和授权 Evidence，以及根级只读 `prompts/`、`insights/`、
`skills/`、可写 `tools/`、
...
| Agent Resources | 版本化 `prompts/`、`insights/`、`skills/` 与 Optimizer 可持续修改的 `tools/`，各自维护 README 索引 | 检查完成后的 Evidence，整理四目录，并把成熟 Tool 沉淀为新 Revision 的 Skill |
