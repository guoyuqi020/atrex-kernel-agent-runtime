...
if [[ "$(uname -s)" != "Linux" ]]; then
  echo "The Evolver sandbox example requires Linux." >&2
  exit 69
fi
if (( EUID != 0 )); then
  ...
  exec sudo --preserve-env=AGATE_URL,AGATE_GPU,ATREX_PYTHON,ATREX_RUNTIME_CLI,ATREX_EVOLVER_AGENT_BACKEND,ATREX_SANDBOX_WORKER_USER,ATREX_SANDBOX_HOST_HOME,ATREX_SANDBOX_HOST_PATH,ANTHROPIC_AUTH_TOKEN,ANTHROPIC_API_KEY,ANTHROPIC_BASE_URL,ANTHROPIC_MODEL,CODEX_HOME,QODER_PERSONAL_ACCESS_TOKEN \
    -- bash "$0" "$@"
fi
...
temporary_state="$(mktemp -d "${TMPDIR:-/tmp}/atrex-evolver-dev-shell.XXXXXX")"
chmod 0711 "${temporary_state}"
export ATREX_EVOLVER_DEV_SHELL_STATE_DIR="${temporary_state}"
...
cleanup_workspace() {
  ...
  if [[ -d "${temporary_state}" ]]; then
    ...
    if ! rm -rf -- "${temporary_state}"; then
      echo "Failed to destroy temporary Evolver workspace: ${temporary_state}" >&2
      status=1
    fi
  fi
  if [[ ! -e "${temporary_state}" ]]; then
    echo "Temporary Evolver workspace destroyed: ${temporary_state}"
  fi
  exit "${status}"
}
...
shell_name="zsh"
backend="${ATREX_EVOLVER_AGENT_BACKEND:-qodercli}"
shell_seen=false
backend_seen=false
for argument in "$@"; do
  case "${argument}" in
    ...
    claude|codex|qodercli|pi)
      if [[ "${backend_seen}" == true ]]; then
        echo "Evolver Backend was specified more than once" >&2
        exit 64
      fi
      backend="${argument}"
      backend_seen=true
...
export ATREX_EVOLVER_AGENT_BACKEND="${backend}"
