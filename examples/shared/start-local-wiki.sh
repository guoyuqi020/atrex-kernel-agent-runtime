...
reference_root="${runtime_root}/local-wiki/corpus/gpu-wiki"
...
if ! "${python_executable}" -c 'import anyio, pydantic, uvicorn' >/dev/null 2>&1; then
  ...
  echo "  python -m pip install -e '.[dev]' -e './local-wiki[dev]'" >&2
...
if [[ ! -d "${reference_root}" ]]; then
  ...
  echo "the Runtime checkout is incomplete; restore local-wiki/corpus" >&2
  exit 2
fi

exec env \
  PYTHONPATH="${runtime_root}/local-wiki/src" \
  "${python_executable}" -m atrex_local_wiki serve \
  --config "${runtime_root}/local-wiki/configs/local.example.json"
