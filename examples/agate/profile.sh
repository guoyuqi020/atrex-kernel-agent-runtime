...
level="${AGATE_PROFILE_LEVEL:-survey}"
if [[ "${level}" == "deep" && -z "${AGATE_KERNEL_NAME:-}" ]]; then
  echo "AGATE_KERNEL_NAME is required when AGATE_PROFILE_LEVEL=deep." >&2
  exit 64
fi
...
arguments=(
  ...
  --bench-iters "${AGATE_BENCH_ITERS:-20}"
  --level "${level}"
  --deps-mode freeze_installed
  --wait
)
if [[ -n "${AGATE_KERNEL_NAME:-}" ]]; then
  arguments+=(--kernel-name "${AGATE_KERNEL_NAME}")
fi
