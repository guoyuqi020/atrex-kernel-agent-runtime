...
1. Initialize the Bundle: `git submodule update --init src/kernel-design-agents`. The current KDA source has no bundled Skills or nested submodules.
...
3. The shipped configs identify the local KDA checkout, an empty submodule URL allowlist, and Bundle limits. [kernel-agent.example.json](../examples/kernel-design-agents/kernel-agent.example.json) is the equivalent reusable configuration section. Set `git_executable` to the absolute path returned by `command -v git`; relative paths resolve from the config directory, not `PATH`.
4. Pin `base_revision.commit` in the new Campaign to a full executable Bundle commit containing `atrex-bundle.json` and `src/main.py`. Commit local changes before selecting that revision. Bootstrap requires the exact commit to exist locally and never downloads missing objects.
