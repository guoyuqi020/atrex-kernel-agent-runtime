...
2. In the Python environment used by workers, install `python3 -m pip install -e .` from the Runtime root. This does not install a local GPU/profiler.
3. The shipped configs identify the local KDA checkout, the declared Skill-submodule URL allowlist, and larger Bundle limits. [kernel-agent.example.json](../examples/kernel-design-agents/kernel-agent.example.json) is the equivalent reusable configuration section. Set `git_executable` to the absolute path returned by `command -v git`; relative paths resolve from the config directory, not `PATH`.
4. Pin `base_revision.commit` in the new Campaign to a full executable Bundle commit containing `atrex-bundle.json` and `src/main.py`. Commit local changes before selecting that revision. Bootstrap requires the exact commit and Skill gitlinks to exist locally and never downloads missing objects.

The importer archives the exact local KDA commit, expands the locally available `ncu-report-skill` gitlink commit, and records its provenance. It performs no fetch, rejects missing local objects, uninitialized or unapproved submodules, links, and further nested submodules, and enforces Bundle limits. An outer `git archive` alone omits the Skill and is not a complete Bundle. Defaults allow 16,384 files and 128 MiB for the Optimizer Bundle. Existing Campaign pins are unchanged. Example preparation replaces the template commit with local KDA HEAD; that HEAD must contain the executable migration, not merely uncommitted working-tree files.
