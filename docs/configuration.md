...
- `reference_projects_root` bound read-only over the Attempt workspace's `reference/` directory
  under those same boundaries; the host check rejects a missing, linked, or empty tree, because the
