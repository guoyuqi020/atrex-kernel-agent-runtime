...
5. 启动 Service 并检查 `/readyz`，完成配置验证。
   ...
   明确的终态基础设施失败（Job `status=failed` 且 `error.error_class=infra`）单独恢复，
   包括日志丢失与 Ray Runtime Environment 初始化失败。
   ...
   和 ABBA 也适用。每个新 Job 都正常绑定或记录提交事件，已完成的同组批次保持不变；
   不消耗 Session Recovery Generation 或 ABBA 单独的结果格式异常重试预算。
   显式 Poll 仍只读；Candidate 编译和正确性失败、未分类错误及已取消的 Job 不重提。
   取消会打断退避等待；网络请求重试和 Agent 错误返回格式不变。
