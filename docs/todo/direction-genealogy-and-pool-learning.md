...
TODO. The current Direction Journal remains authoritative for the lifecycle of each individual
Direction. The relationships proposed here are not yet accepted input fields.
...
Add optional, validated relationships to a Direction without changing its stable identity or
lifecycle:
...
- The Optimizer prompt should explain when to reuse a Direction ID and when to create a derived
  Direction.
- Evolver evidence may summarize relationship graphs, but it must retain links to the underlying
  Reports, Experiments, and Result Artifacts.
- Inspect and export interfaces should expose the graph without requiring conversation parsing.

## Pool analysis enabled by this work

The graph should support separate metrics for:
...
- Inspect output can render the Direction graph for one Epoch or the complete Lineage.
- A cross-Trajectory port is distinguishable from a same-Trajectory continuation.
- Existing Direction records remain readable and valid without migration-authored relationships.
- Tests cover invisible references, cycles where disallowed, duplicate edges, retry recovery, sibling
  ports, and multi-parent combinations.
