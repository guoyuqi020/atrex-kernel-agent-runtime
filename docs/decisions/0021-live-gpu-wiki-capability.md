...
GPU Wiki is an external knowledge source, while lineage experience is Agent-produced local history. Treating one preselected Wiki snapshot as the next Epoch's common memory conflated those responsibilities and prevented an Optimizer from asking a focused question when it became relevant. Returning a live answer without first making it durable would make the Session Trace irreproducible. Immediate feedback after every query would also couple optimization latency to a non-authoritative learning path.
...
Runtime does not inject Wiki selections into cumulative Evidence and does not store lineage experience in Wiki. After an Epoch completes, it prepares `WIKI_FEEDBACK_REPORT` from the frozen interactions, immutable Session Trace references, provider token usage, and bounded Trace projections whose `raw_files` preserve exact captured content. Configured redaction applies only to normalized event summaries. Kernel history, winner facts, component evolution, and lineage memory are excluded. Evidence publication atomically enqueues the report in `wiki_feedback_outbox`. An independent drainer delivers it at least once to `/v1/knowledge/epoch-feedback`; delivery outcome cannot alter optimization, selection, or promotion.
...
model context. Wiki consumption feedback is durable without becoming a synchronous acknowledgement
or an Agent memory system. Real Wiki availability can still affect an individual tool call, so
