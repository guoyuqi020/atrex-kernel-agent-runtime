...
GPU Wiki 是外源知识，lineage Experience 是 Agent 自己生产的本地历史。把一个预先选择的 Wiki Snapshot 当成下一 Epoch 的公共记忆，会混淆两者职责，也使 Optimizer 无法在问题真正出现时发起聚焦查询。若实时响应没有先持久化，Session Trace 将不可复现。每次查询后立即反馈还会让非权威学习路径进入优化延迟。
...
Runtime 不再向累计 Evidence 注入 Wiki Selection，也不把 lineage Experience 存入 Wiki。Epoch 完成后，它根据冻结交互、不可变 Session Trace 引用、Provider Token 用量和受限 Trace Projection 生成 `WIKI_FEEDBACK_REPORT`；其中 `raw_files` 保持捕获内容原样，配置的脱敏只作用于规范化事件摘要。Kernel 历史、胜者事实、组件进化和 lineage 记忆都被排除。Evidence 发布会把 Report 原子插入 `wiki_feedback_outbox`。独立 Drainer 以至少一次语义发送到 `/v1/knowledge/epoch-feedback`；交付结果不能改变优化、选择或晋升。

## 后果

Optimizer 通过 Query 直接获得上游 Wiki 的完整安全 Record 投影，并保存实际影响工作的 Record
稳定 ID。每个模型可见响应都可归因、可重放。外部服务凭据、可信上下文和审计身份保留在模型
上下文之外。Wiki 消费反馈具备持久性，但不会成为同步确认机制，也不会成为 Agent 记忆系统。
真实 Wiki 可用性仍可能影响单次 Tool 调用，因此 Agent 与 Prompt 必须显式处理失败，而不是接收
静默缓存替代。
