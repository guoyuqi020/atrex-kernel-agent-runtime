...
| Evidence | Implemented | Branch-local Optimizer Evidence; promoted-lineage Optimizer chronology; all-completed-branch Evolver chronology with Agent winners, exact Kernel artifacts, outcomes, and raw Session materialization | Retention sizing under long campaigns |
...
| Whole-worker isolation | Implemented for filesystem/process/resources; exact target acceptance pending | Both modes exclude private evaluator paths/data from Worker protocol; production bwrap additionally uses a read-only root/private `~/workspace`, Runtime-storage and sibling-root masking, dropped capabilities/namespaces; system-manager transient service creates/probes roots and runs bwrap as the non-root Worker with cgroup v2 memory/swap/CPU/PID limits; cross-process host-check locking and Worker-native creation cover Lima virtiofs; Workers intentionally share host DNS/routing/services and public egress; repeatable Lima boundary suite and live Claude/Codex/QoderCLI requests pass | Exact production Linux escape/resource/timeout/soak evidence |
