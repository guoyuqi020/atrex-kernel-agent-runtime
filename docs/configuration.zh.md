...
- `reference_projects_root` 在同样的边界约束下只读挂载到 Framework Baseline Workspace 的
  `reference/` 目录；Attempt 不再创建该挂载点，因此拿不到这棵树。参考项目是按需检出的
  Submodule，因此 Host Check 会拒绝缺失、符号链接或空的目录树；
