...
| Runtime State | Agent 执行产生的自适应 `skills/` 与 `tools/`，与版本化源码分开存储。 |
...
Optimizer Session 只读挂载 Agent Source，并提供可写 `skills/`/`tools/`。Runtime 封存每个 Session
终态 State，串行 Attempt 恢复前一 State。Evolution 提供只读 Active/Challenger/Historical Source
与 State，以及可写 Candidate `source/` 和 `runtime-state/{skills,tools}/`；Runtime 校验并将二者
