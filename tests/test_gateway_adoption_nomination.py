...
async def _adopt(history: _History) -> dict[str, Any]:
    ...
    await history.journal("direction_update", request={
        "action": "complete", "direction_id": direction,
    ...
    report["findings"][0]["supporting_experiment_ids"] = [
        report["experiments"][0]["experiment_id"]
    ]
    return report


async def _submit(history: _History, report: dict[str, Any], *, key: str = "nominate") -> Any:
    return await history.service.execute(history.capability.token, json.dumps({
        "schema_version": 2, "attempt_id": history.current.id,
        "idempotency_key": key, "operation": "attempt_report",
        "candidate": _candidate(), "report": report,
    }).encode(), operation_scope="runtime")
...
async def test_duplicate_evaluate_cannot_override_adopted_authority(history: _History) -> None:
    ...
    resolved = history.control.find_candidate_evaluation(
        history.current.id, parse_artifact_digest(history.after.kernel_artifact_digest),
        gateway_result_digest=history.control.list_evaluations(
...
async def test_adopted_nomination_reaches_unchanged_authoritative_gate(
    history: _History, tmp_path: Path, independent: bool,
) -> None:
    report = await _adopt(history)
    await _submit(history, report)
    client = FakeClient()
    ...
    finalizer = AgateAuthoritativeCandidateEvaluator(
        ...
        _builder, FakeContexts(), LocalArtifactStore(tmp_path / "artifacts"),
        history.control, events, wait_timeout_s=100,
        bootstrap_stages=(BootstrapEvaluationStage(1),), clock=lambda: NOW_DATETIME,
    )
    assert history.after.kernel_artifact_digest is not None
...
async def test_visible_bootstrap_with_incompatible_context_cannot_be_adopted(
    ...
    subject = BootstrapGatewaySubject(
        new_attempt_id(), campaign.id, lineage.id, epoch.id,
        history.current.kernel_agent_revision_id,
        campaign.operator, campaign.hardware_target, lineage.dsl,
        campaign.evaluation_contract_digest, digest("seed"), digest("evidence"), NOW_DATETIME,
    )
    changes: dict[str, Any] = {
        "operator": "another_operator", "hardware_target": "another_gpu",
    ...
    capability = history.control.issue_bootstrap(subject, GatewayCapabilityPolicy(
    ...
    assert result.kernel_trial_id is not None
    with pytest.raises(ValueError, match="matching operator, hardware target, DSL"):
        history.control.validate_adoption_trial(history.current.id, result.kernel_trial_id)


@pytest.mark.anyio
@pytest.mark.parametrize("status", ["blocked", "pivot"])
async def test_zero_experiment_report_is_accepted_without_gpu(
    history: _History, status: str,
) -> None:
    report: dict[str, Any] = _value(history.current.id)
    report.update(
        status=status, final_candidate=None,
        blocker="No safe experiment" if status == "blocked" else None,
        experiments=[], findings=[], direction_events=[], profile_evidence=None,
    )
    response = await _submit(history, report)
    assert response.result["status"] == "registered"
...
async def test_empty_report_cannot_omit_an_actual_in_progress_direction(
    ...
    await history.journal("direction_update", request={
        "action": "block" if status == "blocked" else "defer", "direction_id": direction,
...
async def test_adoption_survives_attempt_recovery_without_copying_measurement(
    ...
    history.capability = history.control.issue(history.current.id, GatewayCapabilityPolicy(
        frozenset(GatewayOperation), 4, NOW_DATETIME + timedelta(hours=1),
    ))
...
async def test_empty_report_cannot_hide_inherited_in_progress_direction(history: _History) -> None:
    ...
    for index, event in enumerate(prior["direction_events"][:2]):
        history.control.append_direction_event(
            history.historical_attempt.id, f"old-direction-{index}", event, recovery_generation=0,
        )
    report: dict[str, Any] = _value(history.current.id)
    report.update(
        status="blocked", final_candidate=None, blocker="No safe experiment",
        experiments=[], findings=[], direction_events=[], profile_evidence=None,
    )
    ...
    await history.journal("direction_update", request={
        "action": "block", "direction_id": prior["direction_events"][0]["direction_id"],
