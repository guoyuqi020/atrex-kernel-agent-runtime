...
def write_report(set_name, segments, milestones, deadends, rejected, meta, rows):
    ...
    a("# 切分报告：%s" % set_name)
    a("")
    a("由 `partition.py` 生成。每个 segment 对应一条记录。")
    a("")
    a("| 记录类型 | 数量 | 标记 |")
    a("|---|---:|---|")
    ...
    a("| **合计** | **%d** | |" % len(segments))
    a("")
    a("id 形如 `nvidia.<product>.<dsl>.<slug|family.anti>.<technique>-<marker><NN>`，"
      "编号从已提交记录库现有最大值上方起，以便将来并库免于重编号。"
      "标记用 `%s`/`%s`/`%s` 而非主链的 `s`：本套是 sibling run，"
      "`gain.comparable` 为 false。" % (MARKER_STRATEGY, MARKER_ANTI, MARKER_REFK))
    a("")

    if meta["unit"] == "version-ladder":
        a("## strategy：ratchet 里程碑")
        a("")
        a("选择规则见本 skill 的 `scripts/ladder.py`："
          "一个版本只有在刷新了此前的最好成绩、且改善超过阈值时才算里程碑。"
          "逐步 delta 不可用作进度曲线——这类语料里改善与回归的步数几乎一样多。")
        a("")
        a("| 版本 | kind | geomean us | 改善 | dsl | id 尾段 |")
        a("|---|---|---:|---:|---|---|")
    ...
    a("## anti-strategy：失败与判据")
    a("")
    split = Counter(s.get("split_by") for s in segments
                    if s["record_type"] == "anti-strategy")
    a("| 拆分方式 | 数量 | 说明 |")
    a("|---|---:|---|")
    a("| curated | %d | 运行自己写下的 pitfall 或配置对照，已是独立一条 |"
      % split.get("curated", 0))
    a("| agent | %d | 散文式的失败叙述，正则拆不动，整条交给 agent 按语义拆 |"
      % split.get("agent", 0))
    a("")
    a("回归版本刻意保留：这批语料里 `REVERTED` 版本的根因分析"
      "（`profile_evidence.evidence_chain` + `pitfalls_and_fixes`）"
      "是最密的机制知识，别处没有。")
    a("")
