...
__global__ void k_reduce(const Params* P) {
  ...
  for (int k = 0; k < P->top_k; k++) {
    int slot = t * P->top_k + k;
    // Gather the y16 element unconditionally, BEFORE the router-id fetch, and
    // keep the loop branch-free (select, not continue): a validity `continue`
    // lets the compiler sink the y16 load behind the id test, serializing the
    ...
    float v = bf16_to_f32(P->y16[(long long)slot * P->N + h]);
    ...
    // Matches k_hist/k_scatter validity convention.
    acc += (e >= 0 && e < P->experts) ? (w * v) : 0.0f;
  }
...
class Model(nn.Module):
    ...
    def forward(
        ...
        metas = [
            ...
            ("k_reduce",
             (_cdiv(token_count * hidden_size, 256), 1), (256, 1)),
        ]
