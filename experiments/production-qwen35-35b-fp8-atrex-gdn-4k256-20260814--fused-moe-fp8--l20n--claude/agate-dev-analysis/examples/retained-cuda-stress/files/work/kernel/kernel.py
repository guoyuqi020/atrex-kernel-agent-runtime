...
Pipeline per ``forward`` (all on the current/default stream):
  ...
  5. tiled block-scaled GEMM1: hidden_q @ w1[e]^T  (m16n8k32 e4m3 tensor-core
     MMA, fp32 accumulation, per-128-K-chunk block-scale application)
...
// Single pass: the group is loaded once into registers (16 x uint4 = 256 B),
// amax and quantization both work from registers, output is written as
// 8 x uint4.  Addresses are multiples of 256 B (src) / 128 B (dst), so the
// vector accesses are aligned.
__global__ void k_quant_act(const Params* P) {
  ...
  int t = r / P->top_k;
  const uint4* src =
      (const uint4*)(P->x_hidden + (long long)t * P->K + g * TK);
  uint4 v[TK / 8];
#pragma unroll
  for (int i = 0; i < TK / 8; i++) v[i] = src[i];
  float amax = 0.0f;
#pragma unroll
...
#pragma unroll
    for (int j = 0; j < 8; j++) {
      float q = bf16_to_f32(h[j]) / scale;
      q = fminf(fmaxf(q, -FP8_MAX), FP8_MAX);
      ob[i * 8 + j] = f32_to_e4m3_rn(q);
    }
  }
  uint4* dst = (uint4*)(P->q_out + (long long)r * P->K + g * TK);
#pragma unroll
  for (int i = 0; i < TK / 16; i++) dst[i] = out[i];
}
...
// 5b/7b. Tiled block-scaled FP8 GEMM on tensor cores (fp32 accumulation).
// Same I/O contract as k_gemm: rows come from the per-expert sorted order,
// weights belong to the tile's expert, activation scales apply per 128-K
// group, weight scales per 128x128 block.  The dot products run as
// m16n8k32 e4m3 MMA with fp32 accumulators; per 128-K chunk the partial
// accumulators are folded into the master accumulators multiplied by
// a_scale[row] * w_scale[block].  Fragment layout verified in-pod:
//   a0=(r 0-7, k 0-15) a1=(r 8-15, k 0-15) a2=(r 0-7, k 16-31)
//   a3=(r 8-15, k 16-31); b0/b1: n=lane>>2, k halves 0-15 / 16-31;
//   c0/c1=(lane>>2, (lane&3)*2+{0,1}), c2/c3 rows +8.
#define MMA_TM 64
#define MMA_TN 64
#define MMA_TK 128
#define MMA_LDA (MMA_TK + 16)

static __device__ __forceinline__ unsigned int smem_ld_u32(
    const unsigned char* p) {
  return *reinterpret_cast<const unsigned int*>(p);
}

static __device__ __forceinline__ void mma_prefetch(
    const Params* P, int stage, int kc, int tile_rows, int row_start,
    int n0, int tid, const unsigned char* W,
...
#pragma unroll
  for (int i = 0; i < 4; i++) {
    ...
    unsigned int dst = (unsigned int)__cvta_generic_to_shared(
        As + (stage * MMA_TM + row) * MMA_LDA + col16);
    if (row < tile_rows) {
      long long grow = P->sorted_rows[row_start + row];
      const unsigned char* src = P->a_q + grow * P->K + k0 + col16;
...
#pragma unroll
  for (int i = 0; i < 4; i++) {
    ...
    int col16 = (idx & 7) * 16;
    unsigned int dst = (unsigned int)__cvta_generic_to_shared(
        Bs + (stage * MMA_TN + row) * MMA_LDA + col16);
    const unsigned char* src =
        W + (long long)(n0 + row) * P->K + k0 + col16;
    asm volatile("cp.async.ca.shared.global [%0], [%1], 16;\n"
                 ::"r"(dst), "l"(src));
  }
}

__global__ void k_gemm_mma(const Params* P) {
  int tile = blockIdx.x;
  if (tile >= *P->total_tiles) return;
  int expert = P->desc[2 * tile];
  int row_start = P->desc[2 * tile + 1];
  int tile_rows = min(MMA_TM, P->offsets[expert + 1] - row_start);
  int n0 = blockIdx.y * MMA_TN;
  int tid = threadIdx.y * 32 + threadIdx.x;
  int lane = threadIdx.x;
  int wm = (threadIdx.y >> 1) * 32;
  int wn = (threadIdx.y & 1) * 32;

  const unsigned char* W = P->w + P->w_stride * expert;
  const float* WS = P->w_scale + (long long)P->w_scale_stride * expert;

  __shared__ unsigned char As[MMA_TM * 2][MMA_LDA];
  __shared__ unsigned char Bs[MMA_TN * 2][MMA_LDA];
  unsigned char* Asb = &As[0][0];
  unsigned char* Bsb = &Bs[0][0];
...
#pragma unroll
  ...
  int k_chunks = P->K / MMA_TK;

  mma_prefetch(P, 0, 0, tile_rows, row_start, n0, tid, W, Asb, Bsb);
  asm volatile("cp.async.commit_group;\n");

  for (int kc = 0; kc < k_chunks; kc++) {
    if (kc + 1 < k_chunks) {
      mma_prefetch(P, (kc + 1) & 1, kc + 1, tile_rows, row_start, n0, tid,
                   W, Asb, Bsb);
      asm volatile("cp.async.commit_group;\n");
...
#pragma unroll
    for (int ks = 0; ks < 4; ks++) {
      int ka = ks * 32 + (lane & 3) * 4;
#pragma unroll
      for (int mt = 0; mt < 2; mt++) {
        int ra = wm + mt * 16 + (lane >> 2);
        const unsigned char* Arow = Asb + (stage * MMA_TM + ra) * MMA_LDA;
        unsigned int a0 = smem_ld_u32(Arow + ka);
        unsigned int a1 = smem_ld_u32(Arow + 8 * MMA_LDA + ka);
        unsigned int a2 = smem_ld_u32(Arow + ka + 16);
        unsigned int a3 = smem_ld_u32(Arow + 8 * MMA_LDA + ka + 16);
#pragma unroll
        for (int nt = 0; nt < 4; nt++) {
          int nb = wn + nt * 8 + (lane >> 2);
          const unsigned char* Brow = Bsb + (stage * MMA_TN + nb) * MMA_LDA;
          unsigned int b0 = smem_ld_u32(Brow + ka);
          unsigned int b1 = smem_ld_u32(Brow + ka + 16);
          asm volatile(
    ...
    float sw = WS[((n0 + wn) >> 7) * P->n_groups_k + kc];
#pragma unroll
    for (int mt = 0; mt < 2; mt++) {
      int l0 = wm + mt * 16 + (lane >> 2);
      float s0 = 0.0f, s1 = 0.0f;
      if (l0 < tile_rows) {
        long long g0 = P->sorted_rows[row_start + l0];
        s0 = P->a_scale[g0 * P->n_groups_k + kc] * sw;
      }
      if (l0 + 8 < tile_rows) {
        long long g1 = P->sorted_rows[row_start + l0 + 8];
        s1 = P->a_scale[g1 * P->n_groups_k + kc] * sw;
      }
#pragma unroll
      for (int nt = 0; nt < 4; nt++) {
        acc[mt][nt][0] = fmaf(s0, d[mt][nt][0], acc[mt][nt][0]);
        acc[mt][nt][1] = fmaf(s0, d[mt][nt][1], acc[mt][nt][1]);
        acc[mt][nt][2] = fmaf(s1, d[mt][nt][2], acc[mt][nt][2]);
        acc[mt][nt][3] = fmaf(s1, d[mt][nt][3], acc[mt][nt][3]);
      }
    }
    __syncthreads();
  }
...
#pragma unroll
    for (int half = 0; half < 2; half++) {
      int lr = l0 + half * 8;
      if (lr < tile_rows) {
        long long grow = P->sorted_rows[row_start + lr];
        unsigned short* dst =
            P->y16 + grow * P->N + n0 + wn + (lane & 3) * 2;
#pragma unroll
        for (int nt = 0; nt < 4; nt++) {
          unsigned int lo = f32_to_bf16_rn(acc[mt][nt][half * 2 + 0]);
          unsigned int hi = f32_to_bf16_rn(acc[mt][nt][half * 2 + 1]);
          *(unsigned int*)(dst + nt * 8) = lo | (hi << 16);
        }
      }
    }
  }
...
class Model(nn.Module):
    ...
    def forward(
        ...
        act_scale = torch.empty(rows * (k1 // _GROUP), dtype=torch.float32,
                                device=dev)
        # Per-expanded-row scratch: gate_up (cols 0..n1) then out2 (cols 0..n2).
        # Every phase (GEMM1, silu, GEMM2, reduce) addresses it with the SAME
        # row stride n_buf = max(n1, n2) so each expanded row owns a disjoint
        # region; zero-initialized so unrouted rows (reference leaves them
        # zero) stay zero even for out-of-domain routing ids.
        n_buf = max(n1, n2)
        y16 = torch.zeros(rows * n_buf, dtype=torch.bfloat16, device=dev)
        ...
        launch("k_gemm_mma", (max_tiles, n1 // _TILE_N), (32, 4), stream,
               _pack_params(a_q=act_q.data_ptr(), a_scale=act_scale.data_ptr(),
                            ...
                            N=n_buf, K=k1, n_groups_k=k1 // _GROUP,
                            w_stride=n1 * k1,
        ...
        launch("k_silu_quant", (_cdiv(rows * (k2 // _GROUP), 256), 1),
               ...
               _pack_params(y16=y16.data_ptr(), q_out=inter_q.data_ptr(),
                            ...
                            N=n_buf, K=k2, n_groups_k=k2 // _GROUP,
                            R=rows))
        # 7. GEMM2: [rows, k2] x [experts, n2, k2]^T -> [rows, n2] (bf16)
        launch("k_gemm_mma", (max_tiles, n2 // _TILE_N), (32, 4), stream,
