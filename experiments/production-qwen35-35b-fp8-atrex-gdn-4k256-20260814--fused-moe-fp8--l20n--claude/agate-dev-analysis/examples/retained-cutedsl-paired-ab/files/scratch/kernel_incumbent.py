...
launched sequentially on the current stream by one @cute.jit host function):
  ...
  3. gemm_tc_kernel x1 : block-scaled tensor-core (mma.sync.m16n8k32 e4m3)
                         GEMM for the gate_up projection. The raw FP8 partial
                         sum of every 128-K group is accumulated in registers
                         and folded into the scaled accumulator with
                         a_scale[row, kg] * w_scale[e, n_block, kg].
  4. silu_quant_kernel : SiLU(gate)*up in the reference's rounding order, then
                         ...
                         (written in sorted-position order).
  5. gemm_tc_kernel x2 : tensor-core GEMM for the down projection. Both
                         GEMM epilogues store only each tile's tile_limit
                         valid rows: expert BM-rounding padding rows are
                         computed but never read downstream, so their stores
                         are skipped.
...
map connects them to the per-pair routing weights in the silu/combine stages.
...
def quant_gather_kernel(
    ...
    # produce garbage that silu/combine never read (they index real
    # positions only, through pos_of_pair). Stores are staged through the
    # register fragment fq (fragment->gmem autovec_copy emits 128-bit
    # stores; a direct gmem->gmem copy would emit byte copies), and per
    # store instruction a warp covers 4 contiguous 128-byte segments of one
    # pair row, so coalescing matches the old gather_kernel.
    tidx, _, _ = cute.arch.thread_idx()
    bidx, _, _ = cute.arch.block_idx()
...
def silu_quant_kernel(
    c1: cute.Pointer,           # bfloat16 [pad_cap, N1], position order (gate | up)
    ...
    # Vectorized SiLU(gate)*up + requantization. SUBS_PER_GROUP threads share
    # one 128-group (one warp per pair), SUB_ELEMS elements each; the rounded
    # bf16 values stay in registers between the amax pass and the requant
    # pass. The grid is launched exact (P * (INTER/GROUP) * SUBS_PER_GROUP is
    # a multiple of ELEM_THREADS), so no predication is needed.
    tidx, _, _ = cute.arch.thread_idx()
    bidx, _, _ = cute.arch.block_idx()

    sm_ptr = cute.arch.alloc_smem(
        cutlass.Float32, ELEM_THREADS + ELEM_THREADS // SUBS_PER_GROUP,
        alignment=16,
    )
    s_part = cute.make_tensor(sm_ptr, cute.make_layout(ELEM_THREADS))
    s_scale = cute.make_tensor(
        sm_ptr + ELEM_THREADS,
        cute.make_layout(ELEM_THREADS // SUBS_PER_GROUP),
    )

    g = bidx * ELEM_THREADS + tidx
    subs_per_pair = (INTER // GROUP) * SUBS_PER_GROUP
    pair = g // subs_per_pair
    rem = g % subs_per_pair
    grp = rem // SUBS_PER_GROUP
    sub = rem % SUBS_PER_GROUP
    pos = pos_of_pair[pair]
    e0 = pos * N1 + grp * GROUP + sub * SUB_ELEMS

    g16 = cute.make_tensor(c1 + e0, cute.make_layout(SUB_ELEMS))
    u16 = cute.make_tensor(c1 + e0 + INTER, cute.make_layout(SUB_ELEMS))
    fr_g = cute.make_fragment_like(g16, cutlass.BFloat16)
    fr_u = cute.make_fragment_like(u16, cutlass.BFloat16)
    cute.autovec_copy(g16, fr_g)
    cute.autovec_copy(u16, fr_u)

    # SiLU(gate) * up with the reference rounding order (bfloat16 after the
    # SiLU and after the product), tracking the partial group amax over the
    # rounded bfloat16 values kept in registers.
    fr_v = cute.make_fragment_like(cute.make_layout(SUB_ELEMS), cutlass.BFloat16)
    amax = cutlass.Float32(0.0)
    for i in cutlass.range_constexpr(SUB_ELEMS):
        gf = fr_g[i].to(cutlass.Float32)
        uf = fr_u[i].to(cutlass.Float32)
    ...
    if tidx % SUBS_PER_GROUP == 0:
        ...
        scale = cute.max(m, cutlass.Float32(1e-12)) / cutlass.Float32(FP8_MAX)
        s_scale[gid] = scale
        inter_s[pos * (INTER // GROUP) + grp] = scale
    cute.arch.barrier()

    scale = s_scale[gid]
    fq = cute.make_fragment_like(cute.make_layout(SUB_ELEMS), F8)
    for i in cutlass.range_constexpr(SUB_ELEMS):
        v = fr_v[i].to(cutlass.Float32) / scale
        v = cute.clamp(v, cutlass.Float32(-FP8_MAX), cutlass.Float32(FP8_MAX))
        fq[i] = v.to(F8)
    o16 = cute.make_tensor(
        inter_q + pos * INTER + grp * GROUP + sub * SUB_ELEMS,
...
def moe_pipeline(
    ...
    c1: cute.Pointer,           # bfloat16 [pad_cap * N1]
    ...
    # rasterization in gemm_tc_kernel is branch-free (no partial last group).
    # Extra M tiles carry block_expert == -1 (sentinel) and early-exit.
    ...
    # ---- GEMM 1 (gate_up): C1[BM x BN] tiles over (positions, N1) ----------
    ...
    mC1 = cute.make_tensor(
        c1, cute.make_layout((pad_cap, N1), stride=(N1, 1))
    )
    sA1_layout, sA1_swz = _make_smem_layout(BM, BK, GEMM_STAGES)
    sB1_layout, sB1_swz = _make_smem_layout(BN, BK, GEMM_STAGES)
    tiled_copy_A1 = _make_gmem_tiled_copy(GEMM_THREADS, BK)
    tiled_copy_B1 = _make_gmem_tiled_copy(GEMM_THREADS, BK)
    ...
    gemm_tc_kernel(
        ...
        grid=[grid_m_pad * (N1 // BN), 1, 1], block=[GEMM_THREADS, 1, 1]
    )

    silu_subs = P * (INTER // GROUP) * SUBS_PER_GROUP
    silu_quant_kernel(c1, pos_of_pair, inter_q, inter_s, P).launch(
        grid=[cute.ceil_div(silu_subs, ELEM_THREADS), 1, 1],
        block=[ELEM_THREADS, 1, 1],
    )
...
class Model(nn.Module):
    ...
    def _compile_pipeline(self, device: torch.device) -> None:
        ...
        c1 = fptr(t16(1, torch.bfloat16), cutlass.BFloat16)
    ...
    def forward(
        ...
        c1 = torch.empty(pad_cap, N1, device=device, dtype=torch.bfloat16)
